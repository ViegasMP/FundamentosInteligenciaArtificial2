...
public class GeneticAlgorithm : MetaHeuristic
{
    ...
    //permite alterar os parametros da mutação gaussiana na interface do Unity
    [Header("Gaussian Mutation Parameters")]
    public float mean = 0f;
    public float standard_deviation = 0.5f;
        ...
        while (populationRed.Count < populationSize)
        {
            ...
            if (seedPopulationFromFile)
            {
                ...
                if (populationRed.Count != 0 && populationBlue.Count != 0)
                {
                    ...
                    //passa mean e standart deviation definidos na interface do Unity como parâmetro
                    new_ind_red.Mutate(mutationProbabilityRedPopulation, mean, standard_deviation);
                    new_ind_blue.Mutate(mutationProbabilityBluePopulation, mean, standard_deviation);
                }
    ...
    public override void Step()
    {
        ...
        List<Individual> newPopBlue; //os dois arrays um pra red , outro pro blue
        ...
        for (int i = 0; i < populationSize; i++)
        {
            ...
            //passa mean e standart deviation definidos na interface do Unity como parâmetro
            newPopRed[i].Mutate(mutationProbabilityRedPopulation, mean, standard_deviation);
            newPopBlue[i].Mutate(mutationProbabilityBluePopulation, mean, standard_deviation);
